...
fn daemon_main(args: CliModuleArgs) -> i32 {
    ...
    // TODO(upstream): also check presence of binaries on Linux, etc
    ...
    // Glyphio supervisor parses to clear its "grant Accessibility" banner, and (b) restart the
    // worker so a fresh event-tap is created under trust. On other platforms this never fires.
    #[cfg(target_os = "macos")]
    let accessibility_tick = crossbeam::channel::tick(std::time::Duration::from_secs(2));
    #[cfg(not(target_os = "macos"))]
    let accessibility_tick = crossbeam::channel::never::<std::time::Instant>();
    #[cfg(target_os = "macos")]
    let mut last_accessibility = espanso_mac_utils::check_accessibility();

    loop {
        select! {
          recv(accessibility_tick) -> _ => {
            #[cfg(target_os = "macos")]
            {
              let granted = espanso_mac_utils::check_accessibility();
              if granted != last_accessibility {
                last_accessibility = granted;
                if granted {
                  info!("macOS Accessibility permission is granted");
                  info!("Accessibility granted after startup — restarting worker to enable keystroke detection");
                  restart_worker(&paths, &paths_overrides, exit_notify.clone(), Some(WORKER_START_REASON_MANUAL.to_string()));
                } else {
                  error!("macOS Accessibility permission is NOT granted — keystroke detection is disabled until you grant it");
                }
              }
