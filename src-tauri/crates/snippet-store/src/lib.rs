...
/// How the expansion engine delivers replacement text to the focused application.
///
/// This is product policy rather than snippet content: the default applies to every snippet,
/// while a bundle-ID rule changes delivery only for that application.
#[derive(Debug, Clone, Copy, Default, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
pub enum ExpansionDelivery {
    /// Inject the expansion as individual keyboard events.
    Keys,
    /// Put the expansion on the clipboard and issue Paste. This is Glyphio's reliable default;
    /// unknown values from a future settings file also fail safe to Paste.
    #[default]
    #[serde(other)]
    Paste,
}

impl ExpansionDelivery {
    fn engine_backend(self) -> &'static str {
        match self {
...
/// Device-local expansion delivery preferences used to generate engine configuration.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct ExpansionPolicy {
    pub default_delivery: ExpansionDelivery,
    /// Exact macOS bundle identifier -> delivery. A `BTreeMap` makes generated artifacts stable.
    pub app_overrides: BTreeMap<String, ExpansionDelivery>,
}

impl ExpansionPolicy {
    pub fn delivery_for_bundle_id(&self, bundle_id: &str) -> ExpansionDelivery {
        self.app_overrides
            .get(bundle_id)
            .copied()
...
impl SnippetStore {
    ...
    pub fn create(&self, new: NewSnippet) -> Result<Snippet> {
        ...
        let snippet = Snippet {
            ...
            trigger: normalize_trigger(&new.trigger),
    ...
    pub fn update(&self, id: &str, patch: SnippetUpdate) -> Result<Snippet> {
        ...
        snippet.trigger = normalize_trigger(&patch.trigger);
    ...
    pub fn render_yaml(&self, config_dir: impl AsRef<Path>) -> Result<()> {
        self.render_yaml_with_policy(config_dir, &ExpansionPolicy::default())
    }

    /// Regenerate snippets plus the expansion delivery policy. Policy files are generated from
    /// Settings rather than accepted as hand-authored engine configuration, so removing a rule
    /// also removes its stale app-specific artifact on the next render.
    pub fn render_yaml_with_policy(
        &self,
        ...
        // Write the base config when missing, and MIGRATE files we generated earlier when
        // the template gains new keys (e.g. search_shortcut: OFF) — but never touch a file
        // the user has hand-edited (one that lost our header).
        let default_cfg = cfg_dir.join("default.yml");
        let existing = std::fs::read_to_string(&default_cfg).unwrap_or_default();
        let expected_default = render_default_engine_config(policy);
        if existing.is_empty()
            || (existing.starts_with("# Generated by Glyphio") && existing != expected_default)
        {
            atomic_write(&default_cfg, expected_default.as_bytes())?;
        }
        ...
        remove_generated(&cfg_dir, "00_delivery_")?;
        ...
        for (i, (bundle_id, delivery)) in policy.app_overrides.iter().enumerate() {
            atomic_write(
                &cfg_dir.join(format!("00_delivery_{i}.yml")),
                yaml::render_delivery_config_yaml(bundle_id, *delivery)?.as_bytes(),
            )?;
        }
        Ok(())
    }
...
fn render_default_engine_config(policy: &ExpansionPolicy) -> String {
    format!(
        "# Generated by Glyphio. The engine runs headless as a sidecar;\n\
# the Glyphio app provides the user-facing tray, notifications, and About dialog.\n\
show_icon: false\n\
show_notifications: false\n\
# Glyphio chooses an explicit delivery backend. Clipboard delivery preserves what the user had\n\
# copied and leaves enough time for the target application to consume the expansion first.\n\
backend: {}\n\
preserve_clipboard: true\n\
restore_clipboard_delay: 500\n\
# The engine is built without modulo (no GUI): its own search UI can never open, and its\n\
# default Alt+Space binding would collide with Glyphio's snippet palette. Disable both.\n\
search_shortcut: \"OFF\"\n\
search_trigger: \"OFF\"\n",
        policy.default_delivery.engine_backend()
    )
}

/// Canonical trigger form: lowercase, no whitespace. Expansion is typed inline, where
/// case is easy to fat-finger and spaces would make a trigger untypeable (the engine
/// matches contiguous keystrokes) — normalise at every write path so the store never
/// holds a trigger the engine can't match.
...
mod tests {
    ...
    #[test]
    fn triggers_are_lowercased_and_despaced_on_create_and_update() {
        let store = SnippetStore::open_in_memory().unwrap();
        let s = store
            .create(NewSnippet {
                trigger: ":Sig Block".into(),
                replacement: "x".into(),
                ..Default::default()
            })
            .unwrap();
        assert_eq!(s.trigger, ":sigblock");
        let s = store
            .update(&s.id, SnippetUpdate {
                trigger: "\tMY trig\n".into(),
                replacement: "x".into(),
                ..Default::default()
            })
            .unwrap();
        assert_eq!(s.trigger, "mytrig");
    }
    ...
    #[test]
    fn render_yaml_applies_delivery_precedence_and_cleans_removed_overrides() {
        let store = SnippetStore::open_in_memory().unwrap();
        let dir = tempfile::tempdir().unwrap();
        let mut policy = ExpansionPolicy {
            default_delivery: ExpansionDelivery::Keys,
            ..Default::default()
        };
        policy
            .app_overrides
            .insert("com.apple.TextEdit".into(), ExpansionDelivery::Paste);
        policy
            .app_overrides
            .insert("com.example.Legacy".into(), ExpansionDelivery::Keys);
        ...
        store.render_yaml_with_policy(dir.path(), &policy).unwrap();

        let default = std::fs::read_to_string(dir.path().join("config/default.yml")).unwrap();
        assert!(default.contains("backend: Inject"));
        assert!(default.contains("preserve_clipboard: true"));
        assert!(default.contains("restore_clipboard_delay: 500"));

        // BTreeMap order makes the generated file names deterministic. Each app config matches
        // the engine's macOS bundle-ID `class` exactly, not a similarly named application.
        let text_edit =
            std::fs::read_to_string(dir.path().join("config/00_delivery_0.yml")).unwrap();
        assert!(text_edit.contains("filter_class: ^com\\.apple\\.TextEdit$"));
        assert!(text_edit.contains("backend: Clipboard"));

        policy.app_overrides.clear();
        policy.default_delivery = ExpansionDelivery::Paste;
        store.render_yaml_with_policy(dir.path(), &policy).unwrap();
        assert!(!dir.path().join("config/00_delivery_0.yml").exists());
        assert!(!dir.path().join("config/00_delivery_1.yml").exists());
        let default = std::fs::read_to_string(dir.path().join("config/default.yml")).unwrap();
        assert!(default.contains("backend: Clipboard"));
    }
